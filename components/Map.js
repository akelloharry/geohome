...
export default function Map({
  ...
  useEffect(() => {
    ...
    map.flyTo({ center: targetCenter, zoom: hasActivePass ? 11 : 9.5, essential: true })
    ...
    properties.forEach((p) => {
      ...
      const sponsored = Boolean(p.sponsored || p.is_sponsored)
      const color = sponsored ? '#FBBF24' : available === false ? '#B26A5C' : '#5F8A7B'
      ...
      const title = hasActivePass && showPropertyNames ? (p.title || 'Property Available') : 'Property Available'
      const popupHtml = `<div style="font-family:'Open Sans', sans-serif; font-size:14px; line-height:1.6; max-width:280px; color:#1F2937;">
        ...
        ${showDetailsButton && hasActivePass ? `<a href="${propertyLink}" style="display:inline-flex; align-items:center; justify-content:center; padding:10px 12px; border-radius:9999px; background:#2C6E5C; color:#ffffff; text-decoration:none; font-weight:700; font-size:13px;">View details</a>` : ''}
      </div>`
  ...
  }, [center, properties, pinLocation, draggable, onPinMove, onMarkerClick, onMapClick, bbox, userLocation, hasActivePass, showPropertyNames, showDetailsButton])
  ...
  if (!mapboxgl.accessToken) {
    return <div className={`flex h-96 items-center justify-center rounded bg-cloud-white text-sm text-anchor-gray ${className}`}>Map unavailable (missing token)</div>
  }
