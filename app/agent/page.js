  ...
  const submit = async (e) => {
    ...
    if (error) {
      ...
      if (msg.includes('schema cache') || /Could not find the '.+' column of '.+' in the schema cache/i.test(msg) || msg.includes('could not find')) {
        alert('Submit failed due to Supabase schema cache mismatch. Run this SQL in Supabase SQL editor:\n\nALTER TABLE public.agent_submissions ADD COLUMN IF NOT EXISTS deposit INTEGER;\nNOTIFY pgrst, \'reload schema\';\n\nThen retry submission.')
      } else {
